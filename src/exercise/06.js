...
function useToggle({
  ...
  React.useEffect(() => {
    const missingControllerOnChangeWarning = `Controlled on has to be used with onChange handler, unless it's read-only.`
    warning(
      !readOnly && !hasOnChange && controlledOn,
      missingControllerOnChangeWarning,
    )
  }, [readOnly, hasOnChange, controlledOn])

  function dispatchWithOnChange(action) {
    if (!onIsControlled) {
      dispatch(action) // if controlled, don't need to control and don't need to render
    }
